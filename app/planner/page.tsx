  ...
  if (cities.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-center p-8 bg-gradient-to-br from-slate-50 to-blue-50 text-slate-900">
        ...
        <h2 className="text-3xl font-bold mb-4 text-blue-950">No cities added yet!</h2>
        <p className="text-blue-600/80 mb-8 max-w-md">Your adventure map is empty. Start by creating your first trip.</p>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 sm:p-8 grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8">
        ...
        <aside className="space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-blue-100 p-6 sticky top-24">
            <h3 className="text-xs font-bold text-blue-400 uppercase tracking-widest mb-4">Destinations</h3>
                  ...
                  {days.map((day, dayIndex) => {
                    ...
                    return (
                      <div key={day.toISOString()} className="relative pl-8 sm:pl-10 border-l-2 border-blue-100 pb-10 last:pb-0 last:border-l-0 group">
                        ...
                        <div className="mb-6">
                          <h3 className="text-xl font-bold text-blue-900">Day {dayIndex + 1}</h3>
                        ...
                        <div className="space-y-4">
                          {dayItems.length > 0 ? (
                            ...
                            <div className="p-6 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50 text-center">
                              <p className="text-slate-400 font-medium italic">No plans yet. Time to explore!</p>
                            </div>
                          )}
      ...
      {isAdding && (
        <div className="fixed inset-0 bg-blue-900/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-in zoom-in-95 duration-300 border border-blue-50">
            <h3 className="text-2xl font-bold mb-6 text-blue-950">Add New Item</h3>
            <form onSubmit={submitItem} className="space-y-5">
              <div>
                <label className="block text-sm font-bold text-blue-900 mb-2 uppercase tracking-wider">Title</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-bold text-blue-900 mb-2 uppercase tracking-wider">Type</label>
                <select
                  value={newItem.type}
              ...
              <div>
                <label className="block text-sm font-bold text-blue-900 mb-2 uppercase tracking-wider">Notes</label>
                <textarea
                  value={newItem.notes}
